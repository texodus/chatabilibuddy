...
function XMPPRouter() {
    ...
    this.route = function(id) {
        return function(message) {
            Log.debug("Routing : " + sys.inspect(message, 4).unlines());
...
function XMPPServer() {
	...
	socket.addListener("data", function (data) {
            Do.chain(upstream.process(data), function(messages) { return Do.map(messages, router.route(socketID)); })(function(x) {}, Log.error);  
